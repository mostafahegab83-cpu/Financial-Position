...
// Requires firebase-init.js loaded first (exposes db, incomesCol, debtsCol, expensesCol, metaDoc)

let incomes = [];   // [{id, amount, label, createdAt}]
let debts   = [];
let expenses = [];  // [{id, name, amount, date, createdAt}]
let cycle = { start: "", end: "" };

// Track which rows are currently being edited (by id)
...
function subscribeAll() {
  incomesCol.orderBy("createdAt", "asc").onSnapshot(snap => {
    incomes = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    refreshAll();
  }, err => console.error("incomes:", err));

  debtsCol.orderBy("createdAt", "asc").onSnapshot(snap => {
    debts = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    refreshAll();
  }, err => console.error("debts:", err));

  expensesCol.orderBy("date", "asc").onSnapshot(snap => {
...
function refreshAll() {
  ...
  if (document.getElementById("sumIncome"))   renderSummary();
...
function addIncome() {
  ...
  incomesCol.add({ amount: amt, label: lbl, createdAt: ts() })
...
function saveCycle() {
  ...
  metaDoc.set({ start: s, end: e }).catch(err => alert("Save failed: " + err.message));
}
...
function renderRecordsPage() {
  ...
  if (incomeBody) {
    incomeBody.innerHTML = incomes.length
      ? incomes.map(it => editingIncome.has(it.id) ? `
            <tr>
  ...
  if (expBody) {
    expBody.innerHTML = expenses.length
      ? expenses.map(it => editingExpense.has(it.id) ? `
            <tr>
...
function saveIncome(id) {
  ...
  if (isNaN(amt) || amt <= 0) return alert("Enter a valid amount");
  incomesCol.doc(id).update({ amount: amt, label: lbl })
