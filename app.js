...
function subscribeAll() {
  ...
  cyclesCol.orderBy("start", "asc").onSnapshot(snap => {
...
async function saveCycle() {
  ...
  try {
    ...
    // Archive cycle if not already present
...
function renderSummary() {
  // Income in cycle (uses income.date; falls back to "in cycle" if no date)
  const incCycle = incomes.filter(i => inCycle(i.date || today()));
  const ti = sum(incomes);
  const tiCycle = sum(incCycle);
  const td = sum(debts);
  const te = sum(expenses.filter(e => inCycle(e.date)));
  const cash = tiCycle - te;
  const net = ti - td - te;
  const set = (id, v) => { const el = document.getElementById(id); if (el) el.textContent = fmt(v); };
  set("sumIncome", ti);
  set("sumDebts", td);
...
function renderReportsPage() {
  ...
  // Build list: archived cycles + active cycle (if not already archived)
  ...
  tbody.innerHTML = list.map(c => {
    const inc = sum(incomes.filter(i => {
      const d = i.date || "";
      return d >= c.start && d <= c.end;
    }));
    const exp = sum(expenses.filter(e => {
      const d = e.date || "";
      return d >= c.start && d <= c.end;
    }));
