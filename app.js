...
const collapsedMonths = new Set(); // { "year-month" } months currently collapsed
...
function renderRecords() {
  ...
  for (let m = 1; m <= 12; m++) {
    ...
    const isEditing = list.some(e => editingEntries.has(e.id));
    const hasContent = list.length > 0 || hasDraft;
    const collapsed = collapsedMonths.has(key) && !hasDraft && !isEditing;

    if (collapsed && hasContent) {
      const monthCash = list.reduce((sum, e) => sum + Number(e.cashAmount || 0), 0);
      const monthExp = list.reduce((sum, e) => sum + Number(e.expAmount || 0), 0);
      running = running + monthCash - monthExp;
      rows.push(renderCollapsedMonthRow(m, monthCash, monthExp, running));
    } else {
...
function renderCollapsedMonthRow(month, monthCash, monthExp, running) {
  const rem = Number(running || 0);
  const remCls = rem > 0 ? "pos" : (rem < 0 ? "neg" : "");
  return `<tr class="collapsed-row">${monthCell(month, 1, "+")}
    <td><div class="display-amount">${fmt(monthCash)}</div><div class="display-desc">Total Cash In</div></td>
    <td><div class="display-amount">${fmt(monthExp)}</div><div class="display-desc">Total Expenses</div></td>
    <td class="${remCls}"><strong>${fmt(running)}</strong></td>
    <td></td>
  </tr>`;
}

function monthCell(month, rowspan, symbol) {
  const btn = symbol
    ? `<button class="toggle-btn" onclick="toggleMonth(${month})" title="${symbol === "−" ? "Collapse" : "Expand"} ${MONTH_SHORT[month-1]}">${symbol}</button>`
    : "";
  return `<td class="month-cell" rowspan="${rowspan}">
    <div class="month-name">${btn}${MONTH_SHORT[month-1]}</div>
...
function renderMonthRow(month, entry, isFirst, rowspan, isDraft, isEmpty, running) {
  const monthCellHtml = isFirst ? monthCell(month, rowspan, "−") : "";

  if (isEmpty) {
    const runCls = running > 0 ? "pos" : (running < 0 ? "neg" : "muted");
    return `<tr>${monthCellHtml}
      <td class="muted">—</td>
      <td class="muted">—</td>
  ...
  if (isEditing) {
    return `<tr data-id="${id}">${monthCellHtml}
      <td>
...
function addRow(month) {
  const key = `${selectedYear}-${month}`;
  if (newRows[key]) return; // already has a draft
