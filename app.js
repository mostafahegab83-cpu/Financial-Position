...
let editingIncome = new Set();
let editingDebt = new Set();
let editingExpense = new Set();
...
function renderSummary() {
  ...
  set("sumCash", cash);
  set("sumNet", net);
  const note = document.getElementById("sumCashNote");
  if (note) note.textContent = cash >= 0 ? "Positive cash flow this cycle." : "Spending exceeds income this cycle.";
}
...
function editIncome(i)   { editingIncome.add(i);   renderRecordsPage(); }
function cancelIncome(i) { editingIncome.delete(i); renderRecordsPage(); }
function editDebt(i)     { editingDebt.add(i);     renderRecordsPage(); }
function cancelDebt(i)   { editingDebt.delete(i);   renderRecordsPage(); }
function editExpense(i)  { editingExpense.add(i);  renderRecordsPage(); }
function cancelExpense(i){ editingExpense.delete(i); renderRecordsPage(); }
...
function renderRecordsPage() {
  ...
  if (incomeBody) {
    incomeBody.innerHTML = incomes.length
      ? incomes.map((it, i) => {
          const isEdit = editingIncome.has(i);
          return isEdit ? `
            <tr>
              ...
              <td>
                ...
                <button class="btn" onclick="cancelIncome(${i})">Cancel</button>
              </td>
            </tr>`
          : `
            <tr>
              ...
              <td>${fmt(it.amount)}</td>
              <td>
                <button class="btn" onclick="editIncome(${i})">Edit</button>
  ...
  if (debtBody) {
    debtBody.innerHTML = debts.length
      ? debts.map((it, i) => {
          const isEdit = editingDebt.has(i);
          return isEdit ? `
            <tr>
              ...
              <td>
                ...
                <button class="btn" onclick="cancelDebt(${i})">Cancel</button>
              </td>
            </tr>`
          : `
            <tr>
              ...
              <td>${fmt(it.amount)}</td>
              <td>
                <button class="btn" onclick="editDebt(${i})">Edit</button>
  ...
  if (expBody) {
    expBody.innerHTML = expenses.length
      ? expenses.map((it, i) => {
          const isEdit = editingExpense.has(i);
          return isEdit ? `
            <tr>
              ...
              <td>
                ...
                <button class="btn" onclick="cancelExpense(${i})">Cancel</button>
              </td>
            </tr>`
          : `
            <tr>
              ...
              <td>${fmt(it.amount)}</td>
              <td>
                <button class="btn" onclick="editExpense(${i})">Edit</button>
