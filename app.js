...
function renderRecords() {
  ...
  for (let m = 1; m <= 12; m++) {
    ...
    const totalRowsForMonth = list.length + (hasDraft ? 1 : 0) || 1; // at least 1 row for the month label

    let first = true;
    if (list.length === 0 && !hasDraft) {
      // Empty month row — placeholder with + Add
      rows.push(renderMonthRow(m, null, true /*first*/, 1, /*draft*/false, /*empty*/true));
    } else {
      list.forEach(e => {
        rows.push(renderMonthRow(m, e, first, totalRowsForMonth, false, false));
        first = false;
      });
      if (hasDraft) {
        rows.push(renderMonthRow(m, { id: newRows[key], _draft: true }, first, totalRowsForMonth, true, false));
      }
    }
  }
...
function renderMonthRow(month, entry, isFirst, rowspan, isDraft, isEmpty) {
  const monthCell = isFirst ? `
    <td class="month-cell" rowspan="${rowspan}">
      <div class="month-name">${MONTH_SHORT[month-1]}</div>
      <a href="javascript:void(0)" class="add-link" onclick="addRow(${month})">+ Add</a>
    </td>` : "";

  if (isEmpty) {
    // No entries yet for this month — show empty editable placeholder
    return `<tr>${monthCell}
      <td class="muted">—</td>
      <td class="muted">—</td>
  ...
  const rem = Number(cashAmt || 0) - Number(expAmt || 0);
