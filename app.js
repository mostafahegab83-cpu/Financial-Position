...
function renderRecords() {
  ...
  for (let i = 1; i <= 12; i++) {
    ...
    rows.push(`<tr>
      <td>
        <div><strong>${MONTH_NAMES[i-1]}</strong></div>
        <a href="javascript:void(0)" class="add-link" onclick="quickAdd('${ym}')">+ Add</a>
      </td>
      <td>
        <div class="field-label">Amount</div>
        <input type="number" id="qa-inc-amt-${ym}" placeholder="Amount" step="0.01" class="cell-input"/>
        <div class="field-label">Description</div>
        <input type="text" id="qa-inc-lbl-${ym}" placeholder="Description" class="cell-input"/>
      </td>
      <td>
        <div class="field-label">Amount</div>
        <input type="number" id="qa-exp-amt-${ym}" placeholder="Amount" step="0.01" class="cell-input"/>
        <div class="field-label">Description</div>
        <input type="text" id="qa-exp-name-${ym}" placeholder="Description" class="cell-input"/>
      </td>
      <td class="${remCls}"><strong>${fmt(c.remaining)}</strong></td>
      <td>
        <button class="btn primary" onclick="toggleMonth('${ym}')">${isOpen ? "Hide" : "View"}</button>
        <button class="btn primary" onclick="toggleMonth('${ym}')">Edit</button>
        ${hasData ? `<button class="btn danger" onclick="deleteMonth('${ym}')">Delete</button>` : ""}
      </td>
    </tr>`);
...
function quickAdd(ym) {
  const incAmt = parseFloat(document.getElementById(`qa-inc-amt-${ym}`).value);
  const incLbl = document.getElementById(`qa-inc-lbl-${ym}`).value.trim();
  const expAmt = parseFloat(document.getElementById(`qa-exp-amt-${ym}`).value);
  const expName = document.getElementById(`qa-exp-name-${ym}`).value.trim();
  const date = `${ym}-01`;
  const tasks = [];
  if (!isNaN(incAmt) && incAmt > 0) {
    tasks.push(incomesCol.add({ amount: incAmt, label: incLbl, date, createdAt: ts() }));
  }
  if (!isNaN(expAmt) && expAmt > 0) {
    if (!expName) return alert("Enter a description for the expense");
    tasks.push(expensesCol.add({ name: expName, amount: expAmt, date, createdAt: ts() }));
  }
  if (!tasks.length) return alert("Enter a Cash In or Expense amount");
  Promise.all(tasks)
    .then(() => {
      ["qa-inc-amt","qa-inc-lbl","qa-exp-amt","qa-exp-name"].forEach(p => {
        const el = document.getElementById(`${p}-${ym}`); if (el) el.value = "";
      });
    })
