...
function renderRecords() {
  ...
  for (let i = 1; i <= 12; i++) {
    ...
    const remCls = c.remaining > 0 ? "pos" : (c.remaining < 0 ? "neg" : "");
    const isOpen = expandedMonths.has(ym);
    const defaultDate = `${ym}-01`;
    const hasData = incs.length || exps.length;
    rows.push(`<tr>
      <td>
        <strong>${MONTH_NAMES[i-1]}</strong>
        <div class="month-totals">In: <strong>${fmt(c.ownCashIn)}</strong> · Exp: <strong>${fmt(c.expenses)}</strong></div>
      </td>
      <td>
        <div class="inline-add">
          <input type="number" id="qa-inc-amt-${ym}" placeholder="Amount" step="0.01"/>
          <input type="text" id="qa-inc-lbl-${ym}" placeholder="Description"/>
          <input type="date" id="qa-inc-date-${ym}" value="${defaultDate}"/>
          <button class="btn primary" onclick="quickAddIncome('${ym}')">+ Add</button>
        </div>
      </td>
      <td>
        <div class="inline-add">
          <input type="number" id="qa-exp-amt-${ym}" placeholder="Amount" step="0.01"/>
          <input type="text" id="qa-exp-name-${ym}" placeholder="Description"/>
          <input type="date" id="qa-exp-date-${ym}" value="${defaultDate}"/>
          <button class="btn primary" onclick="quickAddExpense('${ym}')">+ Add</button>
        </div>
      </td>
      <td class="${remCls}"><strong>${fmt(c.remaining)}</strong></td>
      <td>
        <button class="btn" onclick="toggleMonth('${ym}')">${isOpen ? "Hide" : "View"}</button>
        <button class="btn danger" onclick="deleteMonth('${ym}')" ${hasData ? "" : "disabled"}>Delete</button>
      </td>
    </tr>`);
...
function quickAddIncome(ym) {
  const amount = parseFloat(document.getElementById(`qa-inc-amt-${ym}`).value);
  const label = document.getElementById(`qa-inc-lbl-${ym}`).value.trim();
  const date = document.getElementById(`qa-inc-date-${ym}`).value || `${ym}-01`;
  if (isNaN(amount) || amount <= 0) return alert("Enter a valid amount");
  incomesCol.add({ amount, label, date, createdAt: ts() })
    .then(() => {
      document.getElementById(`qa-inc-amt-${ym}`).value = "";
      document.getElementById(`qa-inc-lbl-${ym}`).value = "";
    })
    .catch(e => alert("Save failed: " + e.message));
}
function quickAddExpense(ym) {
  const amount = parseFloat(document.getElementById(`qa-exp-amt-${ym}`).value);
  const name = document.getElementById(`qa-exp-name-${ym}`).value.trim();
  const date = document.getElementById(`qa-exp-date-${ym}`).value || `${ym}-01`;
  if (isNaN(amount) || amount <= 0) return alert("Enter a valid amount");
  if (!name) return alert("Enter a description");
  expensesCol.add({ name, amount, date, createdAt: ts() })
    .then(() => {
      document.getElementById(`qa-exp-amt-${ym}`).value = "";
      document.getElementById(`qa-exp-name-${ym}`).value = "";
    })
    .catch(e => alert("Save failed: " + e.message));
}
function deleteMonth(ym) {
  const incs = incomeOfMonth(ym);
  const exps = expenseOfMonth(ym);
  const total = incs.length + exps.length;
  if (!total) return;
  if (!confirm(`Delete ALL ${total} entries for ${formatMonthLabel(ym)}?`)) return;
  const batch = db.batch();
  incs.forEach(i => batch.delete(incomesCol.doc(i.id)));
  exps.forEach(e => batch.delete(expensesCol.doc(e.id)));
  batch.commit().catch(e => alert("Delete failed: " + e.message));
}
